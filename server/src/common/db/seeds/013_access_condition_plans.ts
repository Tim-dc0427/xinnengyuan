...
export async function seed(knex: Knex): Promise<void> {
  ...
  const premiumPlan = [
    { code: 'annual_irradiance', label: '年均辐照度', unit: 'kWh/㎡·年', enabled: true, op: '>=', threshold: 1550, category: '光伏资源' },
    { code: 'sunshine_hours', label: '年日照小时数', unit: 'h', enabled: true, op: '>=', threshold: 1350, category: '光伏资源' },
    ...
    { code: 'corridor_available', label: '走廊可用性', unit: '', enabled: true, op: '!=', threshold: '不可用', category: '电网条件' },
    { code: 'transmission_line_length_km', label: '接入距离', unit: 'km', enabled: true, op: '<=', threshold: 7, category: '电网条件' },
    { code: 'unit_cost', label: '单位造价', unit: '元/W', enabled: true, op: '<=', threshold: 4.2, category: '投资条件' },
    { code: 'payback_years', label: '投资回收期', unit: '年', enabled: false, op: '<=', threshold: 6, category: '投资条件' },
    { code: 'irr_pct', label: '内部收益率', unit: '%', enabled: true, op: '>=', threshold: 9, category: '投资条件' },
    { code: 'land_type', label: '土地性质', unit: '', enabled: true, op: 'in', threshold: '未利用地,建设用地,草地,农用地', category: '环境条件' },
    { code: 'env_sensitivity', label: '环保敏感性', unit: '', enabled: true, op: '!=', threshold: '敏感', category: '环境条件' },
    { code: 'geohazard_risk', label: '地质灾害风险', unit: '', enabled: true, op: '!=', threshold: '高', category: '环境条件' },
  ]
