...
function openReport() { reportVisible.value = true }
function exportPDF() {
    ...
    <template v-if="selectedProject">
      ...
      <template v-if="feasibility">
        ...
        <div class="chart-panel" style="text-align:center">
          <el-button type="primary" size="small" @click="openReport">生成评估报告</el-button>
        </div>
      </template>
    </template>

    <!-- 报告预览弹窗 -->
    <el-dialog title="并网可行性评估报告" v-model="reportVisible" width="800px" top="2vh">
      <div id="feasibility-report" style="font-size:13px;color:#303133;line-height:1.8">
        ...
        <div v-if="feasibility">
          ...
          <h4 style="border-bottom:2px solid #F56C6C;padding-bottom:4px;margin-bottom:8px" v-if="constraints.length > 0">三、制约因素分析</h4>
          <table style="width:100%;border-collapse:collapse;margin-bottom:16px" v-if="constraints.length > 0">
          ...
          <div v-else><h4 style="border-bottom:2px solid #67C23A;padding-bottom:4px;margin-bottom:8px">三、制约因素分析</h4><p style="color:#67C23A">所有维度均通过阈值，无制约因素。</p></div>
      ...
      <template #footer>
        <el-button size="small" @click="reportVisible = false">关闭</el-button>
