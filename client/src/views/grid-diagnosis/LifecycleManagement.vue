...
const mainTab = ref('lifecycle')
const lifecycleSubTab = ref('runtime')
...
const filteredEquipment = computed(() => {
  ...
  return equipmentList.value.filter(e => e.equipmentType === selectedEquipmentType.value)
})
  ...
  <div class="page-container">
    ...
    <div class="chart-panel">
      <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
        ...
        <template v-if="equipmentList.length">
          ...
          <span>设备分类：</span>
          <el-select v-model="selectedEquipmentType" placeholder="选择分类" clearable style="width:140px" size="small" @change="onTypeChange">
            <el-option v-for="t in equipmentTypes" :key="t" :label="typeLabelMap[t] || t" :value="t" />
          </el-select>
          <span>选择设备：</span>
          <el-select v-model="selectedEquipmentId" placeholder="选择设备" clearable style="width:240px" size="small" :disabled="!selectedEquipmentType" @change="selectEquipment">
            <el-option v-for="e in filteredEquipment" :key="e.equipmentId" :label="e.equipmentName" :value="e.equipmentId" />
          </el-select>
        </template>
      </div>
    </div>
    ...
    <div class="chart-panel" style="padding:0">
      <el-tabs v-model="mainTab">
        <el-tab-pane label="设备寿命周期管理" name="lifecycle">
          <el-tabs v-model="lifecycleSubTab" style="padding:0 16px 12px">
            <el-tab-pane label="实时运行数据" name="runtime">
              <div v-if="activeSnapshot && selectedStation" style="padding-top:8px">
              ...
              <div v-else style="color:#c0c4cc;text-align:center;padding:60px">请先选择电站查看实时运行数据</div>
            </el-tab-pane>
            <el-tab-pane label="设备铭牌参数" name="nameplate">
              <div v-if="selectedEquipment" style="padding-top:8px">
              ...
              <div v-else style="color:#c0c4cc;text-align:center;padding:60px">请先选择设备查看铭牌参数</div>
            </el-tab-pane>
            <el-tab-pane label="历史事件记录" name="events">
              <div v-if="selectedEquipment" style="padding-top:8px">
                <el-table v-if="lifecycleEvents.length" :data="lifecycleEvents" stripe size="small" max-height="400">
                ...
                <div v-else style="color:#c0c4cc;text-align:center;padding:60px">暂无历史事件记录</div>
              </div>
              <div v-else style="color:#c0c4cc;text-align:center;padding:60px">请先选择设备查看历史事件</div>
            </el-tab-pane>
            <el-tab-pane label="设备剩余寿命预测" name="predict">
              <div v-if="selectedEquipment" style="padding-top:8px">
              ...
              <div v-else style="color:#c0c4cc;text-align:center;padding:60px">请先选择设备查看寿命预测</div>
            </el-tab-pane>
          </el-tabs>
        </el-tab-pane>
        <el-tab-pane label="设备更换计划管理" name="replacement">
          <div style="padding:12px 16px">
            ...
            <div v-else-if="!planning" style="color:#c0c4cc;text-align:center;padding:40px">点击"生成更换计划"按钮查看结果</div>
          </div>
        </el-tab-pane>
      </el-tabs>
    </div>
