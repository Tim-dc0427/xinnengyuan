...
import { fetchStations, fetchStationsSnapshot, detectBackfeed, fetchEquipmentCapacity, fetchEquipmentReliability } from '@/api/grid-diagnosis'
...
const equipmentList = ref<Array<{
  id: string; name: string; type: string; capacityKva: number; voltageKv: number
...
function onMapSelect(id: string | null) {
  ...
  if (id) loadBackfeedEvents(id)
  else backfeedEvents.value = []
}
...
async function openEquipmentDrawer() {
  ...
  try {
    const equipData = await fetchEquipmentCapacity({ stationId: activeStationId.value })
    equipmentList.value = equipData.map((e) => ({
      id: e.equipmentId, name: e.equipmentName, type: e.equipmentType,
      capacityKva: e.ratedCapacityKva, voltageKv: e.ratedVoltageKv,
    }))
...
<template>
  <div class="page-container">
    ...
    <!-- 选中电站实时快照 -->
    <div v-if="activeStation && activeSnapshot" class="chart-panel">
    ...
    <el-drawer v-model="drawerVisible" title="设备台账与实时运行状态" direction="rtl" size="600px">
      ...
      <div v-for="eq in equipmentList" :key="eq.id" style="margin-bottom:12px;border:1px solid #e4e7ed;border-radius:4px;padding:10px 12px">
        ...
        <el-descriptions :column="2" size="small" border>
          <el-descriptions-item label="额定容量">{{ eq.capacityKva.toFixed(0) }} kVA</el-descriptions-item>
