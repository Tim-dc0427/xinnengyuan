...
## 开发铁律：先对齐现有接口/数据名称，再动手写功能

**涉及数据改动或新增功能时，第一步必须是搞清楚当前代码里已有的接口名称、字段名、数据表名，对齐后再动手，禁止直接开始写逻辑。**

### 为什么

接口名称和字段名不一致是返工的第一大原因。新功能写到一半发现调错接口、用了不存在的字段、自己编了不一致的命名——浪费的时间远超先花 5 分钟读完代码的时间。

### 强制流程

1. **读已有的接口定义**：`client/src/api/` 下同名文件的函数签名和参数名、`packages/shared/src/types/` 下相关类型定义
2. **读已有的后端路由**：`server/src/modules/<模块>/` 下的路由路径和 controller 方法名
3. **读已有的数据表结构**：相关迁移文件中表的列名和类型
4. **对齐命名**：新代码的接口调用、字段引用与现有代码保持一致
5. **才开始写逻辑**

### 禁止行为

- ❌ 不知道现有接口名就自己编一个调
- ❌ 不读 shared 类型就自己定义 `any` 类型的参数
- ❌ 不查数据表列名就用自己猜的字段名写查询
- ❌ 不看已有 API 封装就直接 `apiClient` 裸调

## 开发铁律：每次打包部署包前必须检查前后端是否最新

**打包部署包到桌面时，禁止直接用旧 dist / 旧源码打包。必须先验证最新再打。**

### 强制流程

1. `git status` 确认工作区是否有未提交改动
2. 前端 dist 最后构建时间 vs 源码最后修改时间 — 源码更新则必须 `npx vite build` 重新构建
3. 后端源码最后修改时间 — 有改动需重新复制到部署目录
4. 确认部署包内 `public/` 的 `index.html` 入口 hash 是当前最新构建产物
5. 全部确认后才开始压缩打包

### 检查清单

- [ ] `ls -la client/dist/index.html` 时间 >= 所有 `client/src/` 文件修改时间？
- [ ] 部署包 `public/index.html` 引用 hash 与 `client/dist/index.html` 一致？
- [ ] `ls -la client/src/api/` 和 `client/src/views/` 无更新于 dist 之后的文件？
- [ ] `git diff --stat` 确认无遗漏的源码改动未反映到 dist？

## 开发铁律：状态切换禁止全量重载

**停用/启用、上线/下线等状态切换操作，API 调用成功后禁止执行 `loadAll()` 等全量数据重载函数。**

### Why
状态切换只改变一条记录的 status 字段，`loadAll()` 会连带刷新所有关联子数据（如设备健康评分），一个 toggle 触发几十次 HTTP 请求。

### 正确做法
1. 状态切换 API 返回更新后的完整记录（含新状态值）
2. 前端直接用返回值更新本地行：`if (updated) row.status = updated.status`
3. 硬删除除外——数据已移除，需刷新列表（但只调列表接口，不连带刷新子数据）

## 开发铁律：硬删除/级联删除前必须验证数据库实际结构

**涉及物理删除（硬删除）或级联删除时，必须用 PRAGMA 查询数据库实际列名和外键关系，禁止仅凭迁移文件推断。**

### Why
迁移文件是增量执行的，后续迁移可能重建表、改名、加列。凭迁移文件假设列名会导致 `no such column` 500 错误。`projects` 被 9 张表引用无法直接物理删除，`equipment` 被 4 张额外子表引用。

### 强制流程
1. 用临时 `.cjs` 脚本执行 `PRAGMA table_info('表名')` 确认实际列名
2. 遍历所有表执行 `PRAGMA foreign_key_list('表名')` 找反向引用
3. NOT NULL 外键列 → 只能 DELETE 整行，不能 SET NULL
4. 被多表引用的实体 → 解绑（SET NULL）而非物理删除
5. 验证全流程通过后再写正式代码
...
- **压缩功能优先 WinRAR**：涉及文件压缩时，优先使用 WinRAR 命令行（`rar.exe`）生成 RAR 格式，而非 zip/7z 等其他格式。调用方式：Node.js `child_process.exec` / `spawn` 执行 `rar a` 命令
